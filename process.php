...
//Removing People and their wishlists / gift ideas
if(isset($_POST['remove_people']))
{
	$query="DELETE FROM people WHERE people.id='{$_POST['person_id']}'";
	mysql_query($query);
	$query="DELETE FROM products WHERE people_id='{$_POST['person_id']}'";
	mysql_query($query);
	header('location:rainforest.php#remove_person');
	exit;
}
